...
let $batch:="/bibframe-process/2018-02-06/"
let $comment:="this is missing $node (idx:index completely, not just null "
let $node:="idx:index"

let $query:= cts:and-not-query(
                          cts:and-query((
                                cts:not-query(cts:element-query(xs:QName($node),cts:and-query(()))),
