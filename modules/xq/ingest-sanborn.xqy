...
(: cribbed from ingest-voyager-bib to load sanborn records 
/lscoll/gmd/, using 3rd node: /lscoll/gmd/sanborn/
NOT FINISHED
:)
		...
		let $idx := 
			<mets:dmdSec ID="IDX1">
		        <mets:mdWrap MDTYPE="OTHER">
		            <mets:xmlData>
		                {marcutil:mods-to-idx($mods, <mxe:record/>)}
		            </mets:xmlData>
		        </mets:mdWrap>
		    </mets:dmdSec>    
		let $newmets := 
...
(:store all at root( of /lscoll/pae: not under their own collection, but add subcoll (node3) as a collection too:)
        ...
        let $_x:=xdmp:log(concat($destination-uri, ":",$dest),"notice")
		return 
			($destination-uri, ":",$dest,
            try {
                    (
                        xdmp:document-insert($destination-uri, $newmets,   $permissions , $destination-collections),                            
							xdmp:log(concat($batch, " : ", "New GMD record at: ", $destination-uri) ,"notice")
                    )
            } catch($e) {
                xdmp:log($e, "error")
            }
