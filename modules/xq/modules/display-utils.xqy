...
<properties> 
	...
	<property name="supplementaryContent" range="SupplementaryContent" sort="11pd">Supplementary material</property>
	...
	<property name="itemOf" range="Instance" sort="27g">Holding for</property>
	...
	<property name="metadataAssigner" sort="99a"  range="literal">Cataloger code </property>
 ...
 for $a in $rdf/@*
         return
		 	 ...
		 	 if ($a instance of attribute(rdf:resource) and fn:contains(fn:string($a),"ontologies/b")) then         
				 <dd class="bibdata">{fn:tokenize(fn:string($a),"/")[fn:last()]}</dd>				 
			 else   if (($a instance of attribute(rdf:about) or $a instance of attribute(rdf:resource)) and fn:contains(fn:string($a),"id.loc.gov/resources/") ) then    
				 <dd class="bibdata"><a href="{fn:replace(fn:string($a),$cfg:BF-BASE, $cfg:BF-VARNISH-BASE)}">{fn:string($a)}</a></dd>				    

			 else   if (($a instance of attribute(rdf:about) or $a instance of attribute(rdf:resource) ) and fn:contains(fn:string($a),"id.loc.gov/") ) then  
					...
					(:  <a href="{fn:replace(fn:string($a),$cfg:BF-BASE, $cfg:BF-VARNISH-BASE)}" displayhref="{fn:replace(fn:string($a),$cfg:ID-BASE,'http://mlvlp04.loc.gov:8287')}.displaylabel.html">{fn:string($a)}</a></dd>:)
					return 
				 ...
				 <dd class="bibdata"><a href="{fn:string($a)}">{fn:string($a)}</a></dd>
...
(<img src="" test="{$rdf/rdfs:Resource/bflc:locator/@rdf:resource}" alt="cover image"/>,
for $c in $rdf/bf:SupplementaryContent[fn:matches(fn:string(bf:note/bf:Note/rdfs:label),"cover", "i")]
    ...
    return if ($link != "" ) then 
			...
			,
	for $ppimage in $rdf//rdfs:Resource

		let $link:= ($ppimage/bflc:locator/@rdf:resource | $ppimage/bflc:target/@rdf:resource)[1]
		let $link := if ($link = "" or fn:not($link) and fn:starts-with($ppimage/rdfs:label,"http") ) then
						$ppimage/rdfs:label
					else 
						$link
    ...
    return if ($link != "" ) then 
			<img src="{$link}" style="align:right;"  alt="{$label}"/>
			else ()
)
...
(:for $rdf in $rdf-block/*[fn:not(self::* instance of element (rdf:type))][fn:not(self::* instance of element (rdfs:label))][fn:not(self::* instance of element (bf:code))][fn:not(self::* instance of element (bf:mainTitle))]:)
...
( 
if ($rdf instance of element (bf:instanceOf) or $rdf instance of element (bflc:itemOf)  or 
	$rdf instance of element (bflc:consolidates) or
	$rdf instance of element (bf:hasPart)
 ) then
        ()
    
    else if ($rdf instance of element (bf:subject) and $rdf/* )  then
