			...
			or $bfraw-work/bf:title[1]/bf:Title[fn:not(rdf:type)] (: not all have matchkeys :)
			 ...
			 then	
	        	...
	        	let $found:= if ($search = "try to link" ) then 
								...
								let $_:=xdmp:log(concat ("CORB auths searching for ", $nameTitle),"info")
    	...
    	let $bfwork:=           
    		try {				
    				xdmp:xslt-invoke("/admin/bfi/auths/auth2bibframe2/auth2bibframe2.xsl",document{$marcxml},$params)
    					...
    					(
						$e,
    					xdmp:log(fn:concat("CORB auth : auth2bibframe transform failed for ",$resclean), "info")
						,xdmp:log($e, "info")
    					)				
    		}
...
let $text-relationships:=if  ($bfwork/rdf:RDF/bf:Work/bflc:relationship/bflc:Relationship/bf:relatedTo/bf:Work) then
...
let $relateds:= 
	...
	let $had7xx := if ( $related-7xxs/wrap/*  ) then
						("/bibframe/had7xx/","/bibframe/translations/")
						else ()
	let $textrelations := if ( $text-relation-nodes ) then
						"/bibframe/textrelations/"
						else ()
	
	let $rel-colls:=($haslinks, $expressions,$titlerelations, $had7xx,$textrelations)
	let $bfwork:= if ( $distinct-translations or $distinct-relateds or $related-7xxs or $text-relation-nodes) then
						<rdf:RDF>
							...
							<rdf:type rdf:resource="http://id.loc.gov/ontologies/bflc/Hub"/>
							{
								$bfwork/rdf:RDF/bf:Work/*[fn:not(index-of($sevenxx-properties,fn:name(.)))],
							
								(:$bfwork/*[self::* instance of element (bf:translationOf) or self::* instance of element (bf:relatedTo) or self::* instance of element (bf:hasPart) )],:)
...
return
	if ($deprecated and doc-available($destination-uri)) then
    	...
    	return	if (($doc-exists and  fn:not(fn:doc($new-uri)//bflc:consolidates) ) 
