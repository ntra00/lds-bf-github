...
declare function vf:facet-data($params as xs:string*) {
    ...
    let $result := if ($view-name="Subject") then
						vf:facet-values($ns, $ln, $id, $query-id-long, $cur-params, ("frequency-order", "concurrent", concat("limit=", $cfg:FACETS-PER-BOX), "collation=http://marklogic.com/collation/en/S1"))
					else
						vf:facet-values($ns, $ln, $id, $query-id-long, $cur-params, ("frequency-order", "concurrent", concat("limit=", $cfg:FACETS-PER-BOX), "collation=http://marklogic.com/collation/codepoint"))
    let $_ := 
...
declare function vf:facet-data-more($params, $id) as element(div)+ {
	...
	let $options:= if ($name="Subject") then
            (
                $sort-order,
                $sort-type, 
                "fragment-frequency",
