...
declare variable $REGEX-FOR-RESOURCES-SUGGEST-TOKEN as xs:string := "/resources/(.*)/suggest/(lccn|token|isbn)/(.+)$";
...
(: ========================== experimental redirect of lccn based search =============================:)
					 ...
					 let $accept := if ($ser="xml") then  "application/mdoc+xml" 
					 				...
					 				else if ($ser="xml") then "application/mldoc\+xml"      							
									else if ($ser="nt") then "application/n-triples"     							
									else if ($ser="json") then "application/json"      							
									else if ($ser="jsonld") then "application/ld+json"     							
...
(: handle feed requests:)			
		...
		(:rdf  :)
		      ...
		      let $accept := 
			  					 "application/bf-2marc+xml"							
								

		      let $tmppath := replace($path, "/", "")
		      let $objid := substring-before($tmppath, ".marc-pkg.xml")
		      return(
			   		concat("/lds/permalink.xqy?uri=", $objid, "&amp;mime=", $accept)
			   )	  
