...
return
    if (matches($mime, "(application/xhtml\+xml|text/html)")) then
        ...
        let $facets :=
            if (exists($cfg:DISPLAY-ELEMENTS//*:elt/*:page[text() = $page])) then
                ...
                ()
        return if ($mime eq "json") then
					xdmp:to-json (vr:result($results, ($count + $start - 1), $query))
					

					 (:json:serialize(xdmp:to-json(feed:mets-to-atom($results, $term) )):)
		else
            (
    ...
    (:else if ($mime eq "application/json") then
       		xdmp:to-json ($searchres) :)
	else if ($mime eq "json") then
 			xdmp:to-json (feed:mets-to-atom($results, $term) )
