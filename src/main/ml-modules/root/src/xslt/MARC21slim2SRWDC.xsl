...
<xsl:stylesheet version="1.0" exclude-result-prefixes="marc" default-validation="strip" input-type-annotations="unspecified" xmlns:marc="http://www.loc.gov/MARC21/slim" xmlns:srw_dc="info:srw/schema/1/dc-schema" xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" xmlns="http://purl.org/dc/elements/1.1/" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
...
-->
	...
	<xsl:template match="marc:record" as="item()*">
		...
		<type>
			<xsl:if test="$leader7='c'">
				...
				<xsl:text disable-output-escaping="no">collection</xsl:text>
			</xsl:if>
		...
		<xsl:for-each select="marc:datafield[@tag=020]">
			<identifier>
				<xsl:text disable-output-escaping="no">URN:ISBN:</xsl:text>
				<xsl:value-of select="marc:subfield[@code='a']" disable-output-escaping="no"/>
			</identifier>
		</xsl:for-each>
