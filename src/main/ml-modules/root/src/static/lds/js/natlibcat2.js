...
function toggleFacetBox(img, idStr) {
	if (img.attr('src') == "/static/lds/images/accordion-closed.png") {
		img.attr("src", "/static/lds/images/accordion-open.png");  //up
	} else {
		img.attr("src", "/static/lds/images/accordion-closed.png");  //down
	}
...
function setupSearch() {
	...
	textField.autocomplete({
		...
		source: function (req, add) {
			...
			$.get("/lds/suggest.xqy?term=" + encodeURIComponent(term), function (data) {
				add(data.split(","));
			});
		}
	});
...
$(document).ready( function ($) {
	...
	*/
	...
	$("#ds-marcview").click(function(e){
		...
		e.preventDefault();
		$.ajax({
            url: "/lds/parts/ajax-MARC.xqy?view=ajax&objid=" + objid,
            success: function(data){
                $.facybox(data);
            }
        });
