...
see main function call: function index:mods-to-idx
for version history
:)
...
declare function index:findLastSpace($titleChop as xs:string ) as xs:string{
(:called by gethitlist():)    
     if ( substring($titleChop,string-length($titleChop))!=" ") then 
        index:findLastSpace(substring($titleChop, 1, string-length($titleChop)-1))
        else $titleChop
    };
(:---------------------------------------------------------------------------:)
declare function index:getModsDigitized($mods as element())  as element(wrap) {
...
(:---------------------------------------------------------------------------:)
declare function index:getDigitized($mxe as element(), $holdings as element() )  as element()+ {
(: 
...
(:-----------------------------------------------------------------------:)
declare function index:parseDates( $date as element(),$type as xs:string )  as element() {
...
(:-----------------------------------------------------------------------:)
declare function index:getAboutDates($mods as element() )  as element()? {
...
let $aboutdates:= 
    for $temporalsubject in $mods//mods:subject[mods:temporal]            
                for $date in $temporalsubject/mods:temporal                    
                        return index:parseDates($date,"about")                      
   return 
...
(:-----------------------------------------------------------------------:)
declare function index:getPubDates($origin as element()) as element() {
(: find best date for faceting and sorting :)
...
let $result:=
   if ($range) then
     <idx:range>
		...
		<idx:begyear>{index:gyear($begindate)}</idx:begyear>        
        <idx:endpubdate>{$enddate}</idx:endpubdate>
		<idx:endyear>{index:gyear($enddate)}</idx:endyear>        
     </idx:range>
     else 
        (<idx:beginpubdate>{$begindate}</idx:beginpubdate>,
		<idx:begyear>{index:gyear($begindate)}</idx:begyear>        )
   ...
   <idx:pubdates>
        ...
        {if (exists($computedBegin)) then 
            ...
            return (<idx:pubdateSort>{$sort}</idx:pubdateSort>,
					<idx:pubyrSort>{index:gyear($sort)}</idx:pubyrSort>        )
        else if (exists($begindate)) then        
        	(<idx:pubdateSort>{$begindate}</idx:pubdateSort> ,
			      <idx:pubyrSort>{index:gyear($begindate)}</idx:pubyrSort>        )
        	else ()}        
    </idx:pubdates>
...
(:-----------------------------------------------------------------------:)
declare function index:getCoverage($datefield as element()) as element() {
...
(:********************************** :)
declare function index:getObjectType($mods as element() ,$profile as xs:string )  {
...
(: *************************************************:)
declare function index:getmarccountries($placeCode as xs:string ) {
             ...
             for $placeCode in $placeCodeSet
                return
                    if ($placeCode/@authority="marccountry") then
                        index:getmarccountries($placeCode/string())
                    else 
...
declare function index:languages($lang as element(mods:language)*) as element(idx:language)* {
...
(:********************************** :)
declare function index:getLocations($mods as element(),$hold as element()? ) { 
(:lcc is the model:
				...
				if ($mods//mods:location/mods:physicalLocation) then
                    for $loc in distinct-values($mods//mods:location/mods:physicalLocation)                                  
                         return
						  ...
						  else  <idx:loc><idx:loc1>{index:cleanLocation($loc)}</idx:loc1></idx:loc>
...
(:********************************** :)
declare function index:getNameTitle($mods as node())  {
...
(: *************************************************:)
declare function index:getTitles($mods as element(), $membership ) {
...
(: *************************************************:)
declare function index:getNames($mods as element() , $membership as xs:string) {
(: may have more than one per term per doc :)
...
declare function index:getMattype($mxe as element()  ) as element()+ {
...
declare function index:getModsMattype($mods as element()  ) as element()+ {
...
(: *************************************************:)
declare function index:getTopics($mods as element() ) as element()* {
(: may have more than one per term per doc :)
(: not finished:)
...
(: *************************************************:)
declare function index:getModsPub($mods as element() ){
let $pubPlace:=index:getPlaces($mods)/idx:pubPlace[1]/string()
(:(index:getPlaces($mods)[name()="idx:pubPlace"])[1]/string():)

let $pubDates:= index:getPubDates($mods/mods:originInfo) 
...
(: *************************************************:)
declare function index:getModsHitlist($mods as element() ) as element() + {
...
let $modstitleText:= 
        if (string-length($modstitle) >200 ) then
                concat(index:findLastSpace(substring($modstitle,1,200)),"...")
...
let $role:= if ($mods/mods:name[1]/mods:role) then
				index:getRole($mods/mods:name[1]/mods:role/mods:roleTerm)
			else ()
let $modscreator880:= 
...
declare function index:getHitlist($mods as element() ,$mxe as element()) as element()+ {
...
let $mxetitleText:= 
        if (string-length($mxetitle) >200 ) then
                concat(index:findLastSpace(substring($mxetitle,1,200)),"... / ",$mxe/mxe:datafield_245/mxe:d245_subfield_c[1])
...
(: *************************************************:)
declare function index:getFacets($mods as element()){
...
let $language:= 
    if ($lang) then index:languages($lang)
...
(:let $locationSet:= 
            if ($mods//mods:location/mods:physicalLocation[not(@xlink:href)]) then
                    (for $loc in distinct-values($mods//mods:location/mods:physicalLocation[not(@xlink:href)])                                  
                          return  index:cleanLocation($loc)
            else "Undetermined"
...
(:********************************** :)
declare function index:getIsbn($isbn as xs:string ) as element()* {
(:
...
(:********************************** :)
declare function index:getIds($mods as element(), $uri as xs:string) as element(){
...
declare function index:mods-to-idx ($mods as element(), $mxe as element()? , $uri as xs:string)  as element()  {  
(:
...
version 20111214 : fixed punctuation,layout of idx:titlesort in index:getTitles added namespace to noindex (indexing updated today)
...
let $holdings:= 
		if ( contains($uri,"erms") ) then 
			utils:hold-bib(tokenize($uri,"\.")[last()]  cast as xs:integer, "erms")
	else  if ((  $mxe/*) and not(matches($mods//mods:recordIdentifier,'[a-zA-Z]') )) then 
			utils:hold-bib($mxe//mxe:controlfield_001/string()  cast as xs:integer, "lcdb")
		else <hld:record/>
  let $digitized:= 		
		if ( not($mxe/*)  or empty($mxe) or not (exists($mxe) or $mxe=() )) then
			index:getModsDigitized($mods)/*
		else						
			index:getDigitized($mxe, $holdings)
    let $display:=     
        if ( not($mxe/*)  or empty($mxe) or not (exists($mxe) or string($mxe)='' )) then
           index:getModsHitlist($mods ) 
        else 
            index:getHitlist($mods,$mxe)
...
(:let $files:=    
    ...
    :)
	let $titles:= index:getTitles($mods, $membership)
    let $names:= index:getNames($mods, $membership) 
    ...
    let $obj:= index:getObjectType($mods, "bibRecord")
