...
<xsl:stylesheet exclude-result-prefixes="marc cinclude" version="1.0" default-validation="strip" input-type-annotations="unspecified" xmlns="http://www.loc.gov/mods/v3" xmlns:marc="http://www.loc.gov/MARC21/slim" xmlns:xlink="http://www.w3.org/1999/xlink" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:cinclude="http://apache.org/cocoon/include/1.0">
	...
	<!--
	MARC21slim2MODS3-
 1.52plus - adapted from official version for ML ingest.  Added role=composer instead of creator for music
 		fixed 086 duplication
						dropped TOC		
						changed 985 to include all, not just ammem
						added 880 support for selected fields
		...
		Revision 1.40 - Dropped <note> for 510 and added <relatedItem type="isReferencedBy"> for 510 2009/11/19 2010/02/19 tmee
	...
	<!-- <xsl:variable name="lccn" select="normalize-space(//marc:datafield[@tag='010']/marc:subfield[@code='a'] )"/> -->
	<!-- this is for single queries: -->
	<!-- <xsl:variable name="holdingsURL">http://lcweb2.loc.gov:8081/diglib/admin/natlib/query1?db=ilssv4&amp;SQL=SELECT BIB_INDEX.NORMAL_HEADING as lccn, LOCATION.Location_code, location_name, location_display_name, location_opac FROM (BIB_INDEX INNER JOIN BIB_LOCATION ON BIB_INDEX.BIB_ID = BIB_LOCATION.BIB_ID) INNER JOIN LOCATION ON BIB_LOCATION.LOCATION_ID = LOCATION.LOCATION_ID WHERE BIB_INDEX.INDEX_CODE='010A' AND BIB_INDEX.NORMAL_HEADING ='<xsl:value-of select="$verifiedLCCN"/>'</xsl:variable>
		<xsl:variable name="holdings" select="document($holdingsURL)"/> -->
	<!-- this is for morgan's batch of 20,000, holdings pre-queried: -->
	<!--<xsl:variable name="holdings" select="document('http://lcweb2.loc.gov/natlib/lscoll/bibs/holdings.xml')/holdings"/>-->
	<!--
		local only:				nate added 991 locations for xml data store 8/5/08
		and toc handling in mods:extention and looking for locations in holdings file
		and 920,925 for selection decision (edeposit)
	-->

	<!-- added for TocHldgs -->
	<xsl:variable name="prefixes" select="document('/db/xslt/bibLCCNPrefixes.xml')/bibLCCNPrefixes"/>
	
	<xsl:variable name="upperLCCN">
		<!-- lowercase and remove blanks -->
		<xsl:value-of select="translate(normalize-space(//marc:datafield[@tag='010']/marc:subfield[@code='a'] ),'abcdefghijklmnopqrstuvwxyz','ABCDEFGHIJKLMNOPQRSTUVWXYZ')" disable-output-escaping="no"/>
	</xsl:variable>
	<xsl:variable name="normalizedLCCN">
		<xsl:call-template name="normalizeLCCN">
			<xsl:with-param name="lccn" select="$upperLCCN"/>
		</xsl:call-template>
	</xsl:variable>
	<xsl:variable name="verifiedLCCN">
		<xsl:call-template name="verifyPrefix">
			<xsl:with-param name="lccn">
				<xsl:value-of select="$normalizedLCCN" disable-output-escaping="no"/>
			</xsl:with-param>
		</xsl:call-template>
	</xsl:variable>
	...
	<xsl:template match="/" as="item()*">
		<!-- removed error and robots blah blah -->
		<xsl:choose>
			<xsl:when test="//marc:collection">
				<modsCollection xsi:schemaLocation="http://www.loc.gov/mods/v3 http://www.loc.gov/standards/mods/v3/mods-3-3.xsd" xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance">
			...
			<xsl:otherwise>
				<mods version="3.3" xsi:schemaLocation="http://www.loc.gov/mods/v3 http://www.loc.gov/standards/mods/v3/mods-3-3.xsd" xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance">
	...
	<xsl:template name="marcRecord" as="item()*">
			...
			<titleInfo>
				<xsl:choose>
					<xsl:when test="@ind2&gt;0">
						<nonSort>
							<xsl:value-of select="substring($titleChop,1,@ind2)" disable-output-escaping="no"/>
						</nonSort>
						<title>
							<!--<xsl:call-template name="xmllang"/>-->
							<xsl:value-of select="substring($titleChop,@ind2+1)" disable-output-escaping="no"/>
						</title>
					</xsl:when>
					<xsl:otherwise>
						<title>
		...
		<xsl:for-each select="marc:datafield[@tag='210']">
			<titleInfo type="abbreviated">
				<xsl:attribute name="authority">
					<xsl:value-of select="marc:subfield[@code='2']" disable-output-escaping="no"/>
				</xsl:attribute>
				<title>
					<!--<xsl:call-template name="xmllang"/>-->
					<xsl:call-template name="chopPunctuation">
		...
		<xsl:for-each select="marc:datafield[@tag='130'] | marc:datafield[@tag='880'][starts-with(marc:subfield[@code='6'],'130')]|  marc:datafield[@tag='240'] | marc:datafield[@tag='880'][starts-with(marc:subfield[@code='6'],'240')]|  marc:datafield[@tag='730'][@ind2!='2'] | marc:datafield[@tag='880'][starts-with(marc:subfield[@code='6'],'730')]">
			...
			<titleInfo type="uniform">
				<title>
					...
					<xsl:call-template name="chopPunctuation">
						<xsl:with-param name="chopString">
							<xsl:value-of select="substring($str,1,string-length($str)-1)" disable-output-escaping="no"/>
						</xsl:with-param>
					</xsl:call-template>
				</title>
				<xsl:call-template name="part"/>
		...
		<xsl:for-each select="marc:datafield[@tag='100'] | marc:datafield[@tag='880'][starts-with(marc:subfield[@code='6'],'100')]">
			<name type="personal">
				...
				<xsl:variable name="role">
					<xsl:choose>
						<xsl:when test="$leader6='c' or $leader6='d'">composer</xsl:when>
		...
		<xsl:for-each select="marc:datafield[@tag='110'] | marc:datafield[@tag='880'][starts-with(marc:subfield[@code='6'],'110')]">
			<name type="corporate">
		...
		<xsl:for-each select="marc:datafield[@tag='111'] | marc:datafield[@tag='880'][starts-with(marc:subfield[@code='6'],'111')]">
			...
			<name>
				<xsl:if test="@ind1=1">
					<xsl:attribute name="type">
						<xsl:text disable-output-escaping="no">personal</xsl:text>
					</xsl:attribute>
				</xsl:if>
				<namePart>
					<!--<xsl:call-template name="xmllang"/>-->
					<xsl:value-of select="marc:subfield[@code='a']" disable-output-escaping="no"/>
				</namePart>
				<xsl:call-template name="role"/>
			</name>
		</xsl:for-each>
		...
		<originInfo>
			...
			<xsl:if test="translate($MARCpublicationCode,'|','')">
				<place>
					<placeTerm>
						...
						<xsl:value-of select="$MARCpublicationCode" disable-output-escaping="no"/>
					</placeTerm>
				</place>
			</xsl:if>
			...
			<xsl:for-each select="marc:datafield[@tag=260]| marc:datafield[@tag='880'][starts-with(marc:subfield[@code='6'],'260')]">
			...
			-->
			<xsl:for-each select="marc:datafield[@tag=260]| marc:datafield[@tag='880'][starts-with(marc:subfield[@code='6'],'260')]">
				<xsl:for-each select="marc:subfield[@code='b' or @code='c' or @code='g']">
					<xsl:choose>
			...
			<xsl:if test="$controlField008-6='e' and ($leader6='d' or $leader6='f' or $leader6='p' or $leader6='t')">
				<xsl:if test="$controlField008-7-10 and ($controlField008-7-10 != $dataField260c)">
					<dateCreated encoding="marc">
						<!--<xsl:call-template name="xmllang"/>-->
						<xsl:value-of select="concat($controlField008-7-10, $controlField008-11-14)" disable-output-escaping="no"/>
					</dateCreated>
				</xsl:if>
			</xsl:if>

			<xsl:if test="$controlField008-6='e' and not($leader6='d' or $leader6='f' or $leader6='p' or $leader6='t')">
				<xsl:if test="$controlField008-7-10 and ($controlField008-7-10 != $dataField260c)">
			...
			<xsl:if test="$controlField008-6='c' or $controlField008-6='d' or $controlField008-6='i' or $controlField008-6='k' or $controlField008-6='m' or $controlField008-6='u'">
				<xsl:if test="$controlField008-7-10">
					<dateCreated encoding="marc" point="start">
						<!--<xsl:call-template name="xmllang"/>-->
						<xsl:value-of select="$controlField008-7-10" disable-output-escaping="no"/>
					</dateCreated>
				</xsl:if>
			</xsl:if>
			...
			<xsl:if test="$controlField008-6='c' or $controlField008-6='d' or $controlField008-6='i' or $controlField008-6='k' or $controlField008-6='m' or $controlField008-6='u'">
				<xsl:if test="$controlField008-11-14">
					<dateCreated encoding="marc" point="end">
						<!--<xsl:call-template name="xmllang"/>-->
						<xsl:value-of select="$controlField008-11-14" disable-output-escaping="no"/>
					</dateCreated>
				</xsl:if>
			</xsl:if>
		...
		<xsl:if test="$controlField008-35-37">
			<language>
				<languageTerm authority="iso639-2b" type="code">
					<xsl:value-of select="substring($controlField008,36,3)" disable-output-escaping="no"/>
				</languageTerm>
			</language>
		</xsl:if>
					...
					<xsl:when test="../marc:subfield[@code='2']='rfc3066'">
						...
						<xsl:call-template name="rfcLanguages">
							...
							<xsl:with-param name="controlField008-35-37">
								<xsl:value-of select="$controlField008-35-37" disable-output-escaping="no"/>
							</xsl:with-param>
						</xsl:call-template>
					</xsl:when>
					<xsl:otherwise>
						...
						<xsl:variable name="allLanguages">
							<xsl:copy-of select="$langCodes" copy-namespaces="yes"/>
						</xsl:variable>
						<xsl:variable name="currentLanguage">
							<xsl:value-of select="substring($allLanguages,1,3)" disable-output-escaping="no"/>
						</xsl:variable>
						<xsl:call-template name="isoLanguage">
							...
							<xsl:with-param name="remainingLanguages">
								<xsl:value-of select="substring($allLanguages,4,string-length($allLanguages)-3)" disable-output-escaping="no"/>
							</xsl:with-param>
							<xsl:with-param name="usedLanguages">
								<xsl:if test="$controlField008-35-37">
									<xsl:value-of select="$controlField008-35-37" disable-output-escaping="no"/>
		...
		<xsl:variable name="physicalDescription">
			...
			<xsl:variable name="check008-23">
				<xsl:if test="$typeOf008='BK' or $typeOf008='MU' or $typeOf008='SE' or $typeOf008='MM'">
					<xsl:value-of select="true()" disable-output-escaping="no"/>
				</xsl:if>
			</xsl:variable>
			...
			<xsl:if test="marc:datafield[@tag=130]/marc:subfield[@code='h']">
				<form authority="gmd">
					<xsl:call-template name="chopBrackets">
						<xsl:with-param name="chopString">
							<xsl:value-of select="marc:datafield[@tag=130]/marc:subfield[@code='h']" disable-output-escaping="no"/>
						</xsl:with-param>
					</xsl:call-template>
				</form>
			</xsl:if>
			<xsl:if test="marc:datafield[@tag=240]/marc:subfield[@code='h']">
				<form authority="gmd">
					<xsl:call-template name="chopBrackets">
						<xsl:with-param name="chopString">
							<xsl:value-of select="marc:datafield[@tag=240]/marc:subfield[@code='h']" disable-output-escaping="no"/>
						</xsl:with-param>
					</xsl:call-template>
				</form>
			</xsl:if>
			<xsl:if test="marc:datafield[@tag=242]/marc:subfield[@code='h']">
				<form authority="gmd">
					<xsl:call-template name="chopBrackets">
						<xsl:with-param name="chopString">
							<xsl:value-of select="marc:datafield[@tag=242]/marc:subfield[@code='h']" disable-output-escaping="no"/>
						</xsl:with-param>
					</xsl:call-template>
				</form>
			</xsl:if>
			<xsl:if test="marc:datafield[@tag=245]/marc:subfield[@code='h']">
				<form authority="gmd">
					<xsl:call-template name="chopBrackets">
						<xsl:with-param name="chopString">
							<xsl:value-of select="marc:datafield[@tag=245]/marc:subfield[@code='h']" disable-output-escaping="no"/>
						</xsl:with-param>
					</xsl:call-template>
				</form>
			</xsl:if>
			...
			<xsl:for-each select="marc:datafield[@tag='246']/marc:subfield[@code='h'] | marc:datafield[@tag='880'][starts-with(marc:subfield[@code='6'],'246')]/marc:subfield[@code='h']">
				<form authority="gmd">
					<xsl:call-template name="chopBrackets">
						<xsl:with-param name="chopString">
							<xsl:value-of select="." disable-output-escaping="no"/>
			...
			<xsl:if test="marc:datafield[@tag=730]/marc:subfield[@code='h']">
				<form authority="gmd">
					<xsl:call-template name="chopBrackets">
						<xsl:with-param name="chopString">
							<xsl:value-of select="marc:datafield[@tag=730]/marc:subfield[@code='h']" disable-output-escaping="no"/>
						</xsl:with-param>
					</xsl:call-template>
				</form>
			</xsl:if>
		...
		<xsl:if test="string-length(normalize-space($physicalDescription))">
			<physicalDescription>
				<xsl:copy-of select="$physicalDescription" copy-namespaces="yes"/>
			</physicalDescription>
		</xsl:if>
...
<!-- standard notes and untyped notes -->
<!--		<xsl:for-each select="marc:datafield[contains('362, 502,504,511,515,524,533,534,535,536, 538,541,545,546,561,562,581,583,585',@tag)] | marc:datafield[@tag='880'][contains('362,502,504,511,515,524,533,534,535,536,538,541,545,546,561,562,581,583,585',substring(marc:subfield[@code='6'],1,3))]">-->
	<xsl:for-each select="marc:datafield[contains('362, 502,504,511,515,524,533,534,535,536,  538,541,545,546,561,562,581,583,585',@tag) or contains(  '501,507,513,514,516,522,525,526,544,547,550,552,555,556,565,567,580,584,586',@tag  )  ]  | marc:datafield[@tag='880']  [contains(  '362,502,504,511,515,524,533,534,535,536,538,541,545,546,561,562,581,583,585',  substring(marc:subfield[@code='6'],1,3)  )   or contains('501,507,513,514,516,522,525,526,544,547,550,552,555,556,565,567,580,584,586',substring(marc:subfield[@code='6'],1,3)  )]">
			<note>
				<xsl:variable name="type">
					<xsl:choose>
						<xsl:when test="@tag='362'">date/sequential designation</xsl:when>
						<xsl:when test="@tag='502'">thesis</xsl:when>
						<xsl:when test="@tag='504'">bibliography</xsl:when>
						<xsl:when test="@tag='511'">performers</xsl:when>
						<xsl:when test="@tag='515'">numbering</xsl:when>
						<xsl:when test="@tag='524'">preferred citation</xsl:when>
						<xsl:when test="@tag='533'">reproduction</xsl:when>
						<xsl:when test="@tag='534'">original version</xsl:when>
						<xsl:when test="@tag='535'">original location</xsl:when>
						<xsl:when test="@tag='536'">funding</xsl:when>
						<xsl:when test="@tag='538'">system details</xsl:when>
						<xsl:when test="@tag='541'">acquisition</xsl:when>
						<xsl:when test="@tag='545'">biographical/historical</xsl:when>
						<xsl:when test="@tag='546'">language</xsl:when>
						<xsl:when test="@tag='561'">ownership</xsl:when>
						<xsl:when test="@tag='562'">version identification</xsl:when>
						<xsl:when test="@tag='581'">publications</xsl:when>
						<xsl:when test="@tag='583'">action</xsl:when>
		...
		<!-- added for TocHldgs.xsl -->
		<!-- selection decision-->
		<xsl:apply-templates select="marc:datafield[@tag='925' or @tag='920']"/>
		<!-- end addition -->
			...
			<subject>
				<xsl:for-each select="marc:subfield[@code='a' or @code='b' or @code='c']">
					<geographicCode>
						<xsl:attribute name="authority">
							<xsl:if test="@code='a'">
								<xsl:text disable-output-escaping="no">marcgac</xsl:text>
							</xsl:if>
							<xsl:if test="@code='b'">
								<xsl:value-of select="following-sibling::marc:subfield[@code=2]" disable-output-escaping="no"/>
							</xsl:if>
							<xsl:if test="@code='c'">
								<xsl:text disable-output-escaping="no">iso3166</xsl:text>
							</xsl:if>
						</xsl:attribute>
						<xsl:value-of select="self::marc:subfield" disable-output-escaping="no"/>
					</geographicCode>
				</xsl:for-each>
			</subject>
		</xsl:for-each>
		...
		<xsl:for-each select="marc:datafield[@tag=045 and @ind1='2'][marc:subfield[@code='b' or @code='c']]">
			<subject>
			...
			<xsl:for-each select="marc:subfield[@code='b']">
				<classification authority="lcc">
					<xsl:if test="../marc:subfield[@code='3']">
						<xsl:attribute name="displayLabel">
							<xsl:value-of select="../marc:subfield[@code='3']" disable-output-escaping="no"/>
						</xsl:attribute>
					</xsl:if>
					<xsl:value-of select="preceding-sibling::marc:subfield[@code='a'][1]" disable-output-escaping="no"/>
					<xsl:text disable-output-escaping="no"> </xsl:text>
					<xsl:value-of select="text()" disable-output-escaping="no"/>
				</classification>
			</xsl:for-each>
		...
		<xsl:for-each select="marc:datafield[@tag=086][@ind1!=0 and @ind1!=1]">
			<classification>
				<xsl:if test="marc:subfield[@code='2']">
				<xsl:attribute name="authority">
					<xsl:value-of select="marc:subfield[@code='2']" disable-output-escaping="no"/>
		...
		<xsl:for-each select="marc:datafield[@tag='490'][@ind1=0] | marc:datafield[@tag='880'][@ind1=0][starts-with(marc:subfield[@code='6'],'490')] ">
			<relatedItem type="series">
				<titleInfo>
					<title>
						<!--<xsl:call-template name="xmllang"/>-->
		...
		<xsl:for-each select="marc:datafield[@tag=510]">
			<relatedItem type="isReferencedBy">
				<titleInfo>
					<title>
						<xsl:value-of select="concat(marc:subfield[@code='a'],' ', marc:subfield[@code='c'])" disable-output-escaping="no"/>
					</title>
				</titleInfo>
				<xsl:for-each select="marc:subfield[@code='b']">
					<originInfo>
			...
			<relatedItem>
				...
				<name type="corporate">
					...
					<xsl:if test="normalize-space($tempNamePart)">
						<namePart>
							<!--<xsl:call-template name="xmllang"/>-->
							<xsl:value-of select="$tempNamePart" disable-output-escaping="no"/>
						</namePart>
					</xsl:if>
					<xsl:call-template name="role"/>
				</name>
		...
		<xsl:for-each select="marc:datafield[@tag=740][@ind2=2]|    marc:datafield[@tag='880'][@ind2=2][starts-with(marc:subfield[@code='6'],'740')]">
			<relatedItem>
				<xsl:call-template name="constituentOrRelatedType"/>
				<titleInfo>
					<title>
		...
		<!-- added for TocHldgs.xsl -->
		<!-- ammem memberships -->
		<xsl:apply-templates select="marc:datafield[@tag='985']"/>
		<!-- end addition -->
		...
		<xsl:for-each select="marc:datafield[@tag='022']">
			...
			<xsl:if test="marc:subfield[@code='l']">
				...
				<identifier type="issn-l">
					<xsl:value-of select="marc:subfield[@code='l']" disable-output-escaping="no"/>
				</identifier>
			</xsl:if>
		</xsl:for-each>
		...
		<xsl:for-each select="marc:datafield[@tag='010']">
			...
			<identifier type="lccn">
				<xsl:value-of select="normalize-space(marc:subfield[@code='a'])" disable-output-escaping="no"/>
			</identifier>
		</xsl:for-each>
		...
		<xsl:for-each select="marc:datafield[@tag='856'][marc:subfield[@code='u']]">
			<xsl:if test="starts-with(marc:subfield[@code='u'],'urn:hdl') or starts-with(marc:subfield[@code='u'],'hdl') or starts-with(marc:subfield[@code='u'],'http://hdl.loc.gov') ">
				<identifier>
					...
					<xsl:value-of select="concat('hdl:',substring-after(marc:subfield[@code='u'],'http://hdl.loc.gov/'))" disable-output-escaping="no"/>
				</identifier>
			</xsl:if>
		...
		<!--<xsl:variable name="holdingsURL">http://lcweb2.loc.gov:8081/diglib/admin/natlib/query1?db=ilssv4&amp;SQL=SELECT%20BIB_INDEX.NORMAL_HEADING%20as%20lccn,%20LOCATION.Location_code,%20location_name,%20location_display_name,%20location_opac%20FROM%20(BIB_INDEX%20INNER%20JOIN%20BIB_LOCATION%20ON%20BIB_INDEX.BIB_ID%20=%20BIB_LOCATION.BIB_ID)%20INNER%20JOIN%20LOCATION%20ON%20BIB_LOCATION.LOCATION_ID%20=%20LOCATION.LOCATION_ID%20WHERE%20BIB_INDEX.INDEX_CODE='010A'%20AND%20BIB_INDEX.NORMAL_HEADING%20='<xsl:value-of select="$verifiedLCCN"/>'</xsl:variable>

		
		<xsl:variable name="holdings" select="document($holdingsURL)/database"/>
		<xsl:if test="not($holdings) or contains($holdings/results,'Error in query')">
			<xsl:apply-templates select="marc:datafield[@tag=991]"/>
		</xsl:if>
		
			<location>
				<physicalLocation>
					<xsl:choose>
						<xsl:when test="LOCATION_DISPLAY_NAME='.Electronic Journal'">Electronic Journals</xsl:when>
						<xsl:when test="LOCATION_DISPLAY_NAME">
							<xsl:value-of select="LOCATION_DISPLAY_NAME"/>
						</xsl:when>
						<xsl:when test="LOCATION_NAME">
		...
		<xsl:for-each select="marc:datafield[@tag=856][@ind2!=2][marc:subfield[@code='u']]">
			...
			<xsl:when test="@ind2='1' and          count(ancestor::marc:record//marc:datafield[@tag='856'][@ind2='0'])=0 and          count(preceding-sibling::marc:datafield[@tag='856'][@ind2='1'])=0">true</xsl:when>
					...
					<xsl:value-of select="marc:subfield[@code='u']" disable-output-escaping="no"/>
				</url>
			</location>
		</xsl:for-each>
		...
		<!-- added for TocHldgs -->
		<!-- nate added html contents as mods:extension xhtml-->
		<!--<xsl:for-each select="marc:datafield[@tag=856][(contains(marc:subfield[@code='u'],'catdir')  )]">			
			<xsl:variable name="site">
				<xsl:choose>
					<xsl:when test="contains(marc:subfield[@code='u'],'catdir') and contains(marc:subfield[@code='u'],'loc.gov')">
						<xsl:value-of select="concat('http://catdir.loc.gov/',substring-after(marc:subfield[@code='u'],'.gov/'))"/>
					</xsl:when>
					<xsl:otherwise>
			...
			<extension xmlns="http://www.w3.org/1999/xhtml">
				<xsl:if test="marc:subfield[@code='3']">
					<xsl:attribute name="displayLabel">
						<xsl:value-of select="marc:subfield[@code='3']"/>
					</xsl:attribute>
				</xsl:if>				
				<xsl:variable name="tocfileURL" select="concat('http://lcweb2.loc.gov:8081/diglib/admin/xmldb/tidyTOC.html?site=',$site)"/>
				<xsl:variable name="tocfile" select="document($tocfileURL)"/>				
				<htmlrecord>
					<xsl:copy-of select="$tocfile/html/*"/>
				</htmlrecord>				
			</extension>
		</xsl:for-each>-->
		<!-- end addition -->
		...
		<recordInfo>
			...
			<xsl:for-each select="marc:datafield[@tag=040]">
				<xsl:if test="marc:subfield[@code='e']">
					<descriptionStandard>
						<xsl:value-of select="marc:subfield[@code='e']" disable-output-escaping="no"/>
					</descriptionStandard>
				</xsl:if>
			...
			<xsl:for-each select="marc:controlfield[@tag=008]">
				<recordCreationDate encoding="marc">
					<xsl:value-of select="substring(.,1,6)" disable-output-escaping="no"/>
				</recordCreationDate>
			</xsl:for-each>
				...
				<recordIdentifier>
					<xsl:if test="../marc:controlfield[@tag=003]">
						<xsl:attribute name="source">
							<xsl:value-of select="../marc:controlfield[@tag=003]" disable-output-escaping="no"/>
						</xsl:attribute>
					</xsl:if>
					<xsl:value-of select="." disable-output-escaping="no"/>
				</recordIdentifier>
			...
			<!-- 1.44 ntra 2009 -->
			<recordOrigin>Converted from MARCXML to MODS version 3.3 using MARC21slim2MODS3-3TocHldgsML.xsl, a modified version of MARC21slim2MODS3-3.xsl (Revision 1.52) which includes transformation of 880s.</recordOrigin>
	...
	<xsl:template name="displayForm" as="item()*">
		<xsl:for-each select="marc:subfield[@code='c']">
			<displayForm>
				<xsl:value-of select="." disable-output-escaping="no"/>
			</displayForm>
	...
	<xsl:template name="affiliation" as="item()*">
	...
	<xsl:template name="role" as="item()*">
		<xsl:for-each select="marc:subfield[@code='e']">
			<role>
				<roleTerm type="text">
					<xsl:value-of select="." disable-output-escaping="no"/>
	...
	<xsl:template name="part" as="item()*">
	...
	<xsl:template name="relatedPart" as="item()*">
		<xsl:if test="@tag=773">
			<xsl:for-each select="marc:subfield[@code='g']">
				<part>
					<text>
	...
	<xsl:template name="relatedPartNumName" as="item()*">
		<xsl:variable name="partNumber">
			<xsl:call-template name="specialSubfieldSelect">
		...
		<xsl:if test="string-length(normalize-space($partNumber))">
			<partNumber>
				<xsl:value-of select="$partNumber" disable-output-escaping="no"/>
			</partNumber>
		</xsl:if>
		<xsl:if test="string-length(normalize-space($partName))">
			<partName>
				<xsl:value-of select="$partName" disable-output-escaping="no"/>
			</partName>
		</xsl:if>
	</xsl:template>
	<xsl:template name="relatedName" as="item()*">
		<xsl:for-each select="marc:subfield[@code='a']">
			<name>
				<namePart>
					<!--<xsl:call-template name="xmllang"/>-->
	...
	<xsl:template name="relatedForm" as="item()*">
		<xsl:for-each select="marc:subfield[@code='h']">
			<physicalDescription>
				<form>
					<xsl:value-of select="." disable-output-escaping="no"/>
	...
	<xsl:template name="relatedExtent" as="item()*">
		<xsl:for-each select="marc:subfield[@code='h']">
			<physicalDescription>
				<extent>
					<xsl:value-of select="." disable-output-escaping="no"/>
	...
	<xsl:template name="relatedNote" as="item()*">
	...
	<xsl:template name="relatedSubject" as="item()*">
	...
	<xsl:template name="relatedIdentifierISSN" as="item()*">
	...
	<xsl:template name="relatedIdentifierLocal" as="item()*">
	...
	<xsl:template name="relatedIdentifier" as="item()*">
	...
	<xsl:template name="relatedItem76X-78X" as="item()*">
	...
	<xsl:template name="subjectGeographicZ" as="item()*">
		<geographic>
			<xsl:call-template name="chopPunctuation">
				<xsl:with-param name="chopString" select="."/>
			</xsl:call-template>
	...
	<xsl:template name="subjectTemporalY" as="item()*">
		<temporal>
			<xsl:call-template name="chopPunctuation">
				<xsl:with-param name="chopString" select="."/>
			</xsl:call-template>
	...
	<xsl:template name="subjectTopic" as="item()*">
		<topic>
			<xsl:call-template name="chopPunctuation">
				<xsl:with-param name="chopString" select="."/>
			</xsl:call-template>
	...
	<xsl:template name="subjectGenre" as="item()*">
		<genre>
			<xsl:call-template name="chopPunctuation">
				<xsl:with-param name="chopString" select="."/>
			</xsl:call-template>
	...
	<xsl:template name="nameABCDN" as="item()*">
		<xsl:for-each select="marc:subfield[@code='a']">
			<namePart>
				<!--<xsl:call-template name="xmllang"/>-->
				<xsl:call-template name="chopPunctuation">
	...
	<xsl:template name="nameABCDQ" as="item()*">
		<namePart>
			<!--<xsl:call-template name="xmllang"/>-->
			<xsl:call-template name="chopPunctuation">
				<xsl:with-param name="chopString">
	...
	<xsl:template name="nameACDEQ" as="item()*">
		<namePart>
			<!--<xsl:call-template name="xmllang"/>-->
			<xsl:call-template name="subfieldSelect">
	...
	<xsl:template name="constituentOrRelatedType" as="item()*">
		<xsl:if test="@ind2=2">
			<xsl:attribute name="type">constituent</xsl:attribute>
		</xsl:if>
	</xsl:template>
	<xsl:template name="relatedTitle" as="item()*">
	...
	<xsl:template name="relatedTitle76X-78X" as="item()*">
		<xsl:for-each select="marc:subfield[@code='t']">
			<titleInfo>
				<title>
					<xsl:call-template name="chopPunctuation">
	...
	<xsl:template name="relatedOriginInfo" as="item()*">
	...
	<xsl:template name="relatedLanguage" as="item()*">
		<xsl:for-each select="marc:subfield[@code='e']">
	...
	<xsl:template name="nameDate" as="item()*">
		<xsl:for-each select="marc:subfield[@code='d']">
			<namePart type="date">
				<xsl:call-template name="chopPunctuation">
					<xsl:with-param name="chopString" select="."/>
	...
	<xsl:template name="subjectAuthority" as="item()*">
		<xsl:if test="@ind2!=4">
			<xsl:if test="@ind2!=' '">
				<xsl:if test="@ind2!=8">
					<xsl:if test="@ind2!=9">
	...
	<xsl:template name="subjectAnyOrder" as="item()*">
	...
	<xsl:template name="specialSubfieldSelect" as="item()*">
	...
	<!-- 3.2 change tmee 6xx $v genre -->
	<xsl:template match="marc:datafield[@tag=600]" as="item()*">
		<subject>
	...
	<xsl:template match="marc:datafield[@tag=610]" as="item()*">
		<subject>
			<xsl:call-template name="subjectAuthority"/>
			<name type="corporate">
	...
	<xsl:template match="marc:datafield[@tag=611]" as="item()*">
		<subject>
			<xsl:call-template name="subjectAuthority"/>
			<name type="conference">
				<namePart>
	...
	<xsl:template match="marc:datafield[@tag=630]" as="item()*">
		<subject>
			<xsl:call-template name="subjectAuthority"/>
			<titleInfo>
				<title>
	...
	<!-- 1.27 648 tmee-->
	<xsl:template match="marc:datafield[@tag=648]" as="item()*">
		<subject>
			<xsl:if test="marc:subfield[@code=2]">
				<xsl:attribute name="authority">
					<xsl:value-of select="marc:subfield[@code=2]" disable-output-escaping="no"/>
	...
	<xsl:template match="marc:datafield[@tag=650]| marc:datafield[@tag='880'][starts-with(marc:subfield[@code='6'],'650')]" as="item()*">
		<subject>
			<xsl:call-template name="subjectAuthority"/>
			<topic>
				<!--<xsl:call-template name="xmllang"/>-->
	...
	<xsl:template match="marc:datafield[@tag=651]" as="item()*">
		<subject>
			<xsl:call-template name="subjectAuthority"/>
			<xsl:for-each select="marc:subfield[@code='a']">
				<geographic>
	...
	<xsl:template match="marc:datafield[@tag=653]| marc:datafield[@tag='880'][starts-with(marc:subfield[@code='6'],'653')]" as="item()*">
		<subject>
			<xsl:for-each select="marc:subfield[@code='a']">
				<topic>
					<!--<xsl:call-template name="xmllang"/>-->
	...
	<xsl:template match="marc:datafield[@tag=656]" as="item()*">
		<subject>
			<xsl:if test="marc:subfield[@code=2]">
				<xsl:attribute name="authority">
					<xsl:value-of select="marc:subfield[@code=2]" disable-output-escaping="no"/>
	...
	<!-- added for TocHldgs -->
	<xsl:template match="marc:datafield[@tag='991']" as="item()*">
		<location>
			<physicalLocation>
				<xsl:value-of select="marc:subfield[@code='b']" disable-output-escaping="no"/>
				<xsl:if test="marc:subfield[@code='t']"> [<xsl:value-of select="marc:subfield[@code='t']" disable-output-escaping="no"/>]</xsl:if>
				<!--<xsl:call-template name="subfieldSelect">
					<xsl:with-param name="codes">bt</xsl:with-param>
					</xsl:call-template>-->
			</physicalLocation>
		</location>
	</xsl:template>
	<xsl:template match="marc:datafield[@tag='920' or @tag='925']" as="item()*">

		<note type="selection decision">
			<xsl:call-template name="subfieldSelect">
				<xsl:with-param name="codes">ab</xsl:with-param>
			</xsl:call-template>
			<xsl:if test="marc:subfield[@code='x']"> [<xsl:value-of select="marc:subfield[@code='x']" disable-output-escaping="no"/>]</xsl:if>
			<!--<xsl:value-of select="normalize-space(.)"/>			-->
		</note>
	</xsl:template>
	<xsl:template match="marc:datafield[@tag='985']" as="item()*">
		<relatedItem type="host">
			<titleInfo>
				<title>
					<xsl:choose>
						<xsl:when test="marc:subfield[@code='a']='GMD/CT'">Cities and Towns</xsl:when>
						<xsl:when test="marc:subfield[@code='a']='GMD/CITYMAP'">City Map</xsl:when>
						<xsl:otherwise>
							<xsl:value-of select="marc:subfield[@code='a']" disable-output-escaping="no"/>
						</xsl:otherwise>
					</xsl:choose>
		...
		<!-- need crosswalk to all codes for map collections -->
	</xsl:template>
	<!-- end addition -->

	<xsl:template name="termsOfAddress" as="item()*">
		<xsl:if test="marc:subfield[@code='b' or @code='c']">
	...
	<xsl:template name="displayLabel" as="item()*">
		<xsl:if test="marc:subfield[@code='i']">
			<xsl:attribute name="displayLabel">
				<xsl:value-of select="marc:subfield[@code='i']" disable-output-escaping="no"/>
			</xsl:attribute>
		</xsl:if>
		<xsl:if test="marc:subfield[@code='3']">
			<xsl:attribute name="displayLabel">
	...
	<xsl:template name="isInvalid" as="item()*">
		<xsl:param name="type"/>
		<xsl:if test="marc:subfield[@code='z'] or marc:subfield[@code='y']  or marc:subfield[@code='m']">
			<identifier>
				...
				<xsl:if test="marc:subfield[@code='z']">
					<xsl:value-of select="marc:subfield[@code='z']" disable-output-escaping="no"/>
				</xsl:if>
				<xsl:if test="marc:subfield[@code='y']">
					<xsl:value-of select="marc:subfield[@code='y']" disable-output-escaping="no"/>
				</xsl:if>
				<xsl:if test="marc:subfield[@code='m']">
					<xsl:value-of select="marc:subfield[@code='m']" disable-output-escaping="no"/>
				</xsl:if>
			</identifier>
		</xsl:if>
	</xsl:template>
	<xsl:template name="subtitle" as="item()*">
		<xsl:if test="marc:subfield[@code='b']">
			<subTitle>
				<xsl:call-template name="chopPunctuation">
					<xsl:with-param name="chopString">
	...
	<xsl:template name="script" as="item()*">
	...
	<xsl:template name="xmllang" as="item()*">
		<xsl:param name="scriptCode"/>
		<xsl:variable name="script">
			<xsl:choose>
				...
				<xsl:when test="$scriptCode=')2'">Hebrew</xsl:when>
				<xsl:when test="$scriptCode='(S'">Greek</xsl:when>
				<xsl:when test="$scriptCode=')1'">East Asian</xsl:when>
			</xsl:choose>
		</xsl:variable>
		<xsl:if test="$controlField008-35-37!='eng' and ancestor::marc:datafield[@tag='880']">
			<xsl:attribute name="xml:lang">
				<xsl:value-of select="$controlField008-35-37" disable-output-escaping="no"/>
			</xsl:attribute>
		</xsl:if>
	...
	<xsl:template name="parsePart" as="item()*">
		...
		<xsl:variable name="sici2">
			<xsl:choose>
				<xsl:when test="starts-with(substring-after(text(),$level1),':')">
					<xsl:value-of select="substring(substring-after(text(),$level1),2)" disable-output-escaping="no"/>
				</xsl:when>
				<xsl:otherwise>
					<xsl:value-of select="substring-after(text(),$level1)" disable-output-escaping="no"/>
				</xsl:otherwise>
			</xsl:choose>
		</xsl:variable>
		<xsl:variable name="level2">
			<xsl:choose>
				<xsl:when test="contains($sici2,':')">
					<!--  2:3<4  -->
					<xsl:value-of select="substring-before($sici2,':')" disable-output-escaping="no"/>
				</xsl:when>
				<xsl:when test="contains($sici2,'&lt;')">
					<!-- 1: 2<4 -->
					<xsl:value-of select="substring-before($sici2,'&lt;')" disable-output-escaping="no"/>
				...
				<xsl:otherwise>
					<xsl:value-of select="$sici2" disable-output-escaping="no"/>
					<!-- 1:2 -->
				</xsl:otherwise>
			</xsl:choose>
		</xsl:variable>
		<xsl:variable name="sici3">
			<xsl:choose>
				<xsl:when test="starts-with(substring-after($sici2,$level2),':')">
					<xsl:value-of select="substring(substring-after($sici2,$level2),2)" disable-output-escaping="no"/>
				</xsl:when>
				<xsl:otherwise>
					<xsl:value-of select="substring-after($sici2,$level2)" disable-output-escaping="no"/>
				</xsl:otherwise>
			</xsl:choose>
		</xsl:variable>
		<xsl:variable name="level3">
			<xsl:choose>
				<xsl:when test="contains($sici3,'&lt;')">
					<!-- 2<4 -->
					<xsl:value-of select="substring-before($sici3,'&lt;')" disable-output-escaping="no"/>
				</xsl:when>
				<xsl:otherwise>
					<xsl:value-of select="$sici3" disable-output-escaping="no"/>
					<!-- 3 -->
				</xsl:otherwise>
			</xsl:choose>
		</xsl:variable>
		...
		<xsl:if test="$level1">
			<detail level="1">
				<number>
					<xsl:value-of select="$level1" disable-output-escaping="no"/>
				</number>
			</detail>
		</xsl:if>
		<xsl:if test="$level2">
			<detail level="2">
				<number>
					<xsl:value-of select="$level2" disable-output-escaping="no"/>
				</number>
			</detail>
		</xsl:if>
		<xsl:if test="$level3">
			<detail level="3">
				<number>
					<xsl:value-of select="$level3" disable-output-escaping="no"/>
				</number>
			</detail>
		</xsl:if>
		<xsl:if test="$page">
			<extent unit="page">
				<start>
					<xsl:value-of select="$page" disable-output-escaping="no"/>
				</start>
			</extent>
		</xsl:if>
	</xsl:template>
	<xsl:template name="getLanguage" as="item()*">
		...
		<xsl:choose>
			...
			<xsl:otherwise>
				<language>
					<languageTerm authority="iso639-2b" type="code">
						<xsl:value-of select="substring($langString,1,3)" disable-output-escaping="no"/>
					</languageTerm>
				</language>
				<xsl:call-template name="getLanguage">
					<xsl:with-param name="langString" select="substring($langString,4,$length)"/>
	...
	<xsl:template name="isoLanguage" as="item()*">
		...
		<xsl:choose>
			...
			<xsl:when test="not(contains($usedLanguages, $currentLanguage))">
				<language>
					...
					<languageTerm authority="iso639-2b" type="code">
						<xsl:value-of select="$currentLanguage" disable-output-escaping="no"/>
					</languageTerm>
				</language>
				<xsl:call-template name="isoLanguage">
					<xsl:with-param name="currentLanguage">
						<xsl:value-of select="substring($remainingLanguages,1,3)" disable-output-escaping="no"/>
					</xsl:with-param>
					<xsl:with-param name="usedLanguages">
						<xsl:value-of select="concat($usedLanguages,$currentLanguage)" disable-output-escaping="no"/>
					</xsl:with-param>
					<xsl:with-param name="remainingLanguages">
						<xsl:value-of select="substring($remainingLanguages,4,string-length($remainingLanguages))" disable-output-escaping="no"/>
					</xsl:with-param>
				</xsl:call-template>
			</xsl:when>
			<xsl:otherwise>
	...
	<xsl:template name="chopBrackets" as="item()*">
		...
		<xsl:if test="substring($string, 1,1)='['">
			<xsl:value-of select="substring($string,2, string-length($string)-2)" disable-output-escaping="no"/>
		</xsl:if>
		<xsl:if test="substring($string, 1,1)!='['">
			<xsl:value-of select="$string" disable-output-escaping="no"/>
		</xsl:if>
	</xsl:template>
	<xsl:template name="rfcLanguages" as="item()*">
	...
	<xsl:template name="dates045b" as="item()*">
		<xsl:param name="str"/>
		<xsl:variable name="first-char" select="substring($str,1,1)"/>
		<xsl:choose>
			<xsl:when test="$first-char ='c'">
				<xsl:value-of select="concat ('-', substring($str, 2))" disable-output-escaping="no"/>
			</xsl:when>
			<xsl:when test="$first-char ='d'">
				<xsl:value-of select="substring($str, 2)" disable-output-escaping="no"/>
			</xsl:when>
			<xsl:otherwise>
				<xsl:value-of select="$str" disable-output-escaping="no"/>
			</xsl:otherwise>
		</xsl:choose>
	</xsl:template>

	<!-- added for TocHldgs -->
	<xsl:template name="normalizeLCCN" as="item()*">
		<!-- returns lccn with no spaces, no hyphen, no slash, and nothing after a slash 
			if there was a hyphen, returns an error if the stuff after the hyphen was not 
			numeric, and pads the number with leading zeroes to 6 digits -->
		<xsl:param name="lccn"/>

		<xsl:variable name="cleanLCCN">
			<xsl:choose>
				<xsl:when test="contains($lccn,'/')">
					<xsl:value-of select="substring-before($lccn,'/')" disable-output-escaping="no"/>
				</xsl:when>
				<xsl:otherwise>
					<xsl:value-of select="$lccn" disable-output-escaping="no"/>
				</xsl:otherwise>
			</xsl:choose>
		</xsl:variable>

		<xsl:choose>
			<xsl:when test="contains($cleanLCCN,'-')">
				<xsl:variable name="afterHyphen" select="number(substring-after($cleanLCCN,'-'))"/>
				<xsl:choose>
					<xsl:when test="number($afterHyphen)">
						<xsl:value-of select="concat(substring-before($cleanLCCN,'-'),format-number($afterHyphen,'000000'))" disable-output-escaping="no"/>
					</xsl:when>
					<xsl:otherwise>Error after hyphen</xsl:otherwise>
				</xsl:choose>
			</xsl:when>
			<xsl:otherwise>
				<xsl:value-of select="$cleanLCCN" disable-output-escaping="no"/>
			</xsl:otherwise>
		</xsl:choose>
	</xsl:template>
	<xsl:template name="verifyPrefix" as="item()*">
		<xsl:param name="lccn"/>
		<xsl:variable name="prefix">
			<xsl:if test="contains('abcdefghijklmnopqrstuvwxyz',substring($lccn,1,1))">
				<xsl:value-of select="substring($lccn,1,1)" disable-output-escaping="no"/>
			</xsl:if>
			<xsl:if test="contains('abcdefghijklmnopqrstuvwxyz',substring($lccn,2,1))">
				<xsl:value-of select="substring($lccn,2,1)" disable-output-escaping="no"/>
			</xsl:if>
			<xsl:if test="contains('abcdefghijklmnopqrstuvwxyz',substring($lccn,3,1))">
				<xsl:value-of select="substring($lccn,3,1)" disable-output-escaping="no"/>
			</xsl:if>
		</xsl:variable>
		<xsl:choose>
			<xsl:when test="$prefix!=''">
				<!-- there is a prefix -->
				<xsl:choose>
					<xsl:when test="$prefixes/row[@bibLCCNPrefix=$prefix]">
						<!-- scan marc bib prefixes document -->
						<!-- valid prefix -->
						<xsl:choose>
							<xsl:when test="string-length($prefix)&lt;3 and string-length($lccn)&lt;12">
								<!-- insert a space between prefix and number -->
								<xsl:value-of select="concat($prefix,'%20', substring-after($lccn, $prefix))" disable-output-escaping="no"/>
							</xsl:when>
							<xsl:otherwise>
								<xsl:value-of select="$lccn" disable-output-escaping="no"/>
							</xsl:otherwise>
					...
					<xsl:otherwise>Invalid prefix error</xsl:otherwise>
				</xsl:choose>
			</xsl:when>
			<xsl:otherwise>
...
<metaInformation>
<scenarios ><scenario default="yes" name="http://lcweb2.loc.gov:8081/diglib/admin/natlib/juilliardMods/603543145.xml" userelativepaths="yes" externalpreview="no" url="http://lcweb2.loc.gov:8081/diglib/admin/natlib/juilliardMarcxml/603543145.xml" htmlbaseurl="" processortype="internal" commandline="" additionalpath="" additionalclasspath="" postprocessortype="none" postprocesscommandline="" postprocessadditionalpath="" postprocessgeneratedext=""/></scenarios><MapperInfo srcSchemaPath="" srcSchemaRoot="" srcSchemaPathIsRelative="yes" srcSchemaInterpretAsXML="no" destSchemaPath="" destSchemaRoot="" destSchemaPathIsRelative="yes" destSchemaInterpretAsXML="no"/>
</metaInformation>
-->
<!-- Stylus Studio meta-information - (c) 2004-2005. Progress Software Corporation. All rights reserved.
<metaInformation>
<scenarios ><scenario default="no" name="Scenario1" userelativepaths="yes" externalpreview="no" url="http://marklogic1.loctest.gov/88137059/marcxml" htmlbaseurl="" outputurl="" processortype="internal" useresolver="no" profilemode="0" profiledepth="" profilelength="" urlprofilexml="" commandline="" additionalpath="" additionalclasspath="" postprocessortype="none" postprocesscommandline="" postprocessadditionalpath="" postprocessgeneratedext="" validateoutput="no" validator="internal" customvalidator=""/><scenario default="no" name="Scenario2" userelativepaths="yes" externalpreview="no" url="http://lcweb2.loc.gov/natlib/util/files/eadbibs.xml" htmlbaseurl="" outputurl="" processortype="internal" useresolver="no" profilemode="0" profiledepth="" profilelength="" urlprofilexml="" commandline="" additionalpath="" additionalclasspath="" postprocessortype="none" postprocesscommandline="" postprocessadditionalpath="" postprocessgeneratedext="" validateoutput="no" validator="internal" customvalidator=""/><scenario default="yes" name="interleaved" userelativepaths="yes" externalpreview="no" url="http://lcweb2.loc.gov/natlib/util/files/interleaved4.xml" htmlbaseurl="" outputurl="" processortype="saxon8" useresolver="no" profilemode="0" profiledepth="" profilelength="" urlprofilexml="" commandline="" additionalpath="" additionalclasspath="" postprocessortype="none" postprocesscommandline="" postprocessadditionalpath="" postprocessgeneratedext="" validateoutput="no" validator="internal" customvalidator=""/></scenarios><MapperMetaTag><MapperInfo srcSchemaPathIsRelative="yes" srcSchemaInterpretAsXML="no" destSchemaPath="http://marklogic1.loctest.gov/88137059/mods" destSchemaRoot="mods:mods" destSchemaPathIsRelative="yes" destSchemaInterpretAsXML="no" ><SourceSchema srcSchemaPath="http://marklogic1.loctest.gov/88137059/marcxml" srcSchemaRoot="marc:record" AssociatedInstance="" loaderFunction="document" loaderFunctionUsesURI="no"/></MapperInfo><MapperBlockPosition></MapperBlockPosition><TemplateContext></TemplateContext></MapperMetaTag>
</metaInformation>
-->
<!-- Stylus Studio meta-information - (c) 2004-2005. Progress Software Corporation. All rights reserved.
<metaInformation>
<scenarios ><scenario default="yes" name="Scenario1" userelativepaths="yes" externalpreview="no" url="http://lccn.loc.gov/12037148/marcxml" htmlbaseurl="" outputurl="" processortype="internal" useresolver="yes" profilemode="0" profiledepth="" profilelength="" urlprofilexml="" commandline="" additionalpath="" additionalclasspath="" postprocessortype="none" postprocesscommandline="" postprocessadditionalpath="" postprocessgeneratedext="" validateoutput="no" validator="internal" customvalidator=""/></scenarios><MapperMetaTag><MapperInfo srcSchemaPathIsRelative="yes" srcSchemaInterpretAsXML="no" destSchemaPath="" destSchemaRoot="" destSchemaPathIsRelative="yes" destSchemaInterpretAsXML="no"/><MapperBlockPosition></MapperBlockPosition><TemplateContext></TemplateContext></MapperMetaTag>
</metaInformation>
-->
