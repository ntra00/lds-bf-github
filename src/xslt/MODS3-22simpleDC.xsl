...
<xsl:stylesheet version="1.0" exclude-result-prefixes="mods" default-validation="strip" input-type-annotations="unspecified" xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" xmlns:mods="http://www.loc.gov/mods/v3" xmlns:dc="http://purl.org/dc/elements/1.1/" xmlns:srw_dc="info:srw/schema/1/dc-schema" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">

<!-- 
Adapted from http://www.loc.gov/standards/mods/MODS3-22simpleDC.xsl, but uses srw-dc so we can have wrappers if needed
and so we are consistent with the marc2dc xsl
...
-->
	...
	<xsl:template match="mods:subject[mods:topic | mods:name | mods:occupation | mods:geographic | mods:hierarchicalGeographic | mods:cartographics | mods:temporal] " as="item()*">
		<dc:subject>
	...
	<xsl:template match="mods:dateIssued | mods:dateCreated | mods:dateCaptured" as="item()*">
		<dc:date>
			<xsl:choose>
	...
	<xsl:template match="mods:typeOfResource" as="item()*">
	...
	<xsl:template match="mods:identifier" as="item()*">
		...
		<xsl:choose>
			<xsl:when test="contains ('isbn issn uri doi lccn uri', $type)">
				<dc:identifier>
					<xsl:value-of select="$type" disable-output-escaping="no"/>: <xsl:value-of select="." disable-output-escaping="no"/>
				</dc:identifier>
			</xsl:when>
			<xsl:otherwise>
				<dc:identifier>
	...
	<xsl:template match="mods:relatedItem[mods:titleInfo | mods:name | mods:identifier | mods:location]" as="item()*">
		<xsl:choose>
	...
	<xsl:template name="name" as="item()*">
		<xsl:variable name="name">
			...
			<xsl:if test="mods:displayForm">
				<xsl:text disable-output-escaping="no"> (</xsl:text>
				<xsl:value-of select="mods:displayForm" disable-output-escaping="no"/>
				<xsl:text disable-output-escaping="no">) </xsl:text>
			</xsl:if>
	...
	<xsl:template match="mods:dateIssued[@point='start'] | mods:dateCreated[@point='start'] | mods:dateCaptured[@point='start'] | mods:dateOther[@point='start'] " as="item()*">
		<xsl:variable name="dateName" select="local-name()"/>
			<dc:date>
				<xsl:value-of select="." disable-output-escaping="no"/>-<xsl:value-of select="../*[local-name()=$dateName][@point='end']" disable-output-escaping="no"/>
			</dc:date>
	</xsl:template>
	
	<xsl:template match="mods:temporal[@point='start']  " as="item()*">
		<xsl:value-of select="." disable-output-escaping="no"/>-<xsl:value-of select="../mods:temporal[@point='end']" disable-output-escaping="no"/>
	</xsl:template>
