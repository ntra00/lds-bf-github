...
yaz-record-conv $AUTH2BFDIR/auths-conv.xml $SOURCE_UNPROCESSED/single/$LCCN.xml |  sed -e "s|idwebvlcp03.loc.gov|id.loc.gov|g" | sed -e "s|mlvlp04.loc.gov:8080|id.loc.gov|g" > $SOURCE_PROCESSED/single/$LCCN.tmp.rdf
xsltproc  $MODULES/graphiphy.xsl  $SOURCE_UNPROCESSED/single/$LCCN.tmp.rdf    > $LCCN.rdf

rm 				$SOURCE_UNPROCESSED/single/$LCCN.xml
rm 				$SOURCE_UNPROCESSED/single/$LCCN.tmp.rdf

chmod 775 		$SOURCE_PROCESSED/single/$LCCN*
chgrp marklogic $SOURCE_PROCESSED/single/$LCCN*
